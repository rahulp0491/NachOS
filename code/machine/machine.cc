...
// initializing all frames to false
	for (i = 0; i < NumPhysPages; i++)
		isFrameAllocated [i] = false;
}
...
bool Machine::isFrameArrayMapped () {
	int i;
	for (i = 0; i < NumPhysPages; i++) {
		if (isFrameAllocated [i] == true)
			return true;
	}
	return false;
}

void Machine::mapFrame (int frame) {
	isFrameAllocated [frame] = true;
}

void Machine::unmapFrame (int frame) {
		isFrameAllocated [frame] = false;
}

int Machine::getNextFrame () {
	int i;
	for (i = 0; i < NumPhysPages; i++) {
		if (isFrameAllocated [i] == false)
			return i;
	}
	return -1;
}